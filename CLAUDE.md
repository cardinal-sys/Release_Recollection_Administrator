...
このリポジトリは **Release Recollection 50キー版〈Administrator〉キーボードファームウェア設定** リポジトリ。
...
**必ず `/home/user/Release_Recollection_Administrator`（大文字 R）を使うこと。**

- `/home/user/Release_Recollection_Administrator` → プロキシ経由クローン。`git commit` の署名が通る ✅
- `/home/user/release_recollection_administrator` → 直接クローン（手動で作った偽物）。署名が通らない ❌
...
### キーマップ変更（小さな調整）
1. `config/Administrator.keymap` を編集
...
## [ SYSTEM LOG ]
- 〈Administrator Awakening〉— Release Recollection 50キー版・最高司祭の建立。Cygnus素体（Dist16384/Cygnus-M-Lkeymouse）を仕様参考として、Cardinal版（42キー）の設計思想を継承して新リポを起源化。シールドは〈Night_Sky_Sword〉（右）/〈Blue_Rose_Sword〉（左）の双剣構成。Phase 1 以降で記憶解放術式を移植予定。