...
const EDITOR_ID = 'administrator_50';
...
const EDITABLE_PATHS = [
  'keymap.yaml',
  'keymap_drawer.yaml',
  'config/Administrator.keymap',
  'config/Administrator.json',
  'config/west.yml',
  ...
  'config/boards/shields/Administrator/Blue_Rose_Sword.conf',
  'config/boards/shields/Administrator/Blue_Rose_Sword.overlay',
  'config/boards/shields/Administrator/Night_Sky_Sword.conf',
  'config/boards/shields/Administrator/Night_Sky_Sword.overlay',
  'config/boards/shields/Administrator/Administrator.dtsi',
  'config/boards/shields/Administrator/Administrator.zmk.yml',
...
const DEFAULT_REPO = 'cardinal-sys/Release_Recollection_Administrator';
...
const state = {
  ...
  layoutData: null,       // QMK info JSON (config/Administrator.json)
...
async function ensureLayoutLoaded() {
  ...
  try {
    if (!state.files.has('config/Administrator.json')) {
      await fetchFile('config/Administrator.json');
    }
    const jf = state.files.get('config/Administrator.json');
...
const LE_BT_CMDS = ['BT_CLR','BT_NXT','BT_PRV','BT_SEL 0','BT_SEL 1','BT_SEL 2'];
...
function serializeDtsiLayerBindings(bindings, originalText) {
  ...
  for (const size of rowSizes) {
    const row = bindings.slice(idx, idx + size)
      .map(b => leBindingToString(b))
      .join('  ');
    rows.push('    ' + row);
    idx += size;
  }
...
function showLayerEditor(path, file) {
  ...
  if (!state.layoutData) {
    ensureLayoutLoaded().then(() => {
      if (state.activePath === path && state.viewMode === 'layer') renderLayerEditor(path, state.files.get(path));
    });
  }
}
...
function leSelectKey(idx) {
  ...
  const file = state.files.get(path);
  renderLayerEditor(path, file);
  leState.selectedIdx = idx; // renderLayerEditor がリセットするので再設定
  // フォームを開く
...
function leBuildParamsUI(behavior, binding) {
  ...
  function makeKeySel(currentVal, id) {
    ...
    catSel.className = 've-select';
    catSel.id = id + '-cat';
    for (const [k, label] of [['letters','文字 A-Z'],['digits','数字 0-9'],['arrows','矢印'],['special','特殊キー'],['functions','F1-F24'],['modifiers','修飾キー'],['symbols','記号']]) {
      const o = document.createElement('option');
      o.value = k; o.textContent = label;
      catSel.appendChild(o);
    }
    const keySel = document.createElement('select');
    keySel.className = 've-select';
    keySel.id = id + '-key';
    ...
    // カテゴリを自動推測
    let initCat = 'letters';
    for (const [k, vals] of Object.entries(LE_ZMK_CATS)) {
      if (vals.includes(currentVal)) { initCat = k; break; }
    }
    catSel.value = initCat;
    fillKeys(initCat);
  ...
  function makeLayerSel(currentVal, id) {
    ...
    sel.className = 've-select';
    sel.id = id;
    LE_LAYER_NAMES.forEach((name, i) => {
      const o = document.createElement('option');
      o.value = String(i); o.textContent = `${i}: ${name}`;
      if (String(i) === String(currentVal)) o.selected = true;
      sel.appendChild(o);
    });
  ...
  function makeModSel(currentVal, id) {
    ...
    sel.className = 've-select';
    sel.id = id;
    for (const v of LE_ZMK_CATS.modifiers) {
      const o = document.createElement('option');
      o.value = v; o.textContent = v;
      if (v === currentVal) o.selected = true;
      sel.appendChild(o);
  ...
  switch (behavior) {
    case 'kp':
      makeKeySel(args[0] || 'A', 'le-kp');
      break;
    case 'mt':
      makeRow('Modifier (hold)', makeModSel(args[0] || 'LEFT_SHIFT', 'le-mt-mod'));
      makeKeySel(args[1] || 'A', 'le-mt-key');
      break;
    case 'lt':
      makeRow('Layer (hold)', makeLayerSel(args[0] || '0', 'le-lt-layer'));
      makeKeySel(args[1] || 'A', 'le-lt-key');
      break;
    case 'mo':
      makeRow('Layer', makeLayerSel(args[0] || '0', 'le-mo-layer'));
      break;
    case 'gesture_mo_kp':
      makeRow('Layer (gesture)', makeLayerSel(args[0] || '0', 'le-gmo-layer'));
      makeKeySel(args[1] || 'A', 'le-gmo-key');
      break;
    case 'smart_num': {
      const inp = document.createElement('input');
      inp.type = 'number'; inp.className = 've-input'; inp.min = '0'; inp.max = '31';
      inp.value = args[0] || '16'; inp.id = 'le-sn-layer';
      inp.addEventListener('input', leUpdatePreview);
      makeRow('Layer', inp);
      break;
    }
    case 'smart_snipe': {
      const inp = document.createElement('input');
      inp.type = 'number'; inp.className = 've-input'; inp.min = '0'; inp.max = '31';
      inp.value = args[0] || '15'; inp.id = 'le-ss-layer';
      inp.addEventListener('input', leUpdatePreview);
      makeRow('Layer', inp);
      break;
    }
    case 'ht_snipe':
      makeRow('Layer (hold)', makeLayerSel(args[0] || '0', 'le-hs-layer'));
      makeKeySel(args[1] || 'TAB', 'le-hs-key');
      break;
    case 'mkp': {
      const sel = document.createElement('select');
      sel.className = 've-select'; sel.id = 'le-mkp-btn';
      for (const v of LE_MKP_OPTIONS) {
        const o = document.createElement('option');
        o.value = v; o.textContent = v;
        if (v === (args[0] || 'MB1')) o.selected = true;
        sel.appendChild(o);
      }
      sel.addEventListener('change', leUpdatePreview);
      makeRow('ボタン', sel);
    ...
    case 'kt':
    case 'key_repeat':
      makeKeySel(args[0] || 'A', `le-${behavior}-key`);
      break;
    case 'tog':
    case 'to':
    case 'sl':
      makeRow('Layer', makeLayerSel(args[0] || '0', `le-${behavior}-layer`));
      break;
    case 'bt': {
      const BT_CMDS = ['BT_CLR','BT_NXT','BT_PRV','BT_SEL 0','BT_SEL 1','BT_SEL 2','BT_SEL 3','BT_SEL 4','BT_DISC 0','BT_DISC 1','BT_DISC 2','BT_CLR_ALL'];
      const cur = args.join(' ') || 'BT_CLR';
      const sel = document.createElement('select');
      sel.className = 've-select'; sel.id = 'le-bt-cmd';
      for (const v of BT_CMDS) {
    ...
    case 'out': {
      const sel = document.createElement('select');
      sel.className = 've-select'; sel.id = 'le-out-sel';
      for (const [i, v] of ['OUT_AUTO','OUT_USB','OUT_BLE'].entries()) {
        const o = document.createElement('option'); o.value = String(i); o.textContent = v;
        if (String(i) === String(args[0] ?? '0')) o.selected = true;
        sel.appendChild(o);
      }
      sel.addEventListener('change', leUpdatePreview);
      makeRow('出力先', sel);
    ...
    case 'ext_power': {
      const sel = document.createElement('select');
      sel.className = 've-select'; sel.id = 'le-ep-sel';
      for (const [i, v] of ['EP_OFF','EP_ON','EP_TOG'].entries()) {
        const o = document.createElement('option'); o.value = String(i); o.textContent = v;
        if (String(i) === String(args[0] ?? '2')) o.selected = true;
        sel.appendChild(o);
      }
      sel.addEventListener('change', leUpdatePreview);
      makeRow('電源', sel);
    ...
    case 'none':
      break;
    case 'custom': {
      const raw = binding?.behavior === '__raw__' ? binding.args[0] : leBindingToString(binding);
      const inp = document.createElement('input');
      inp.type = 'text'; inp.className = 've-input'; inp.id = 'le-custom-val';
      inp.value = raw || ''; inp.placeholder = '例: &kp A, &mt LEFT_SHIFT Q';
      inp.addEventListener('input', leUpdatePreview);
      makeRow('値', inp);
      break;
    }
...
function leCollectBinding() {
  ...
  switch (beh) {
    ...
    case 'bt': {
      const cmd = g('le-bt-cmd');
      return { behavior:'bt', args: cmd.split(' ') };
    }
    ...
    case 'custom': {
      const raw = g('le-custom-val').trim();
      return { behavior:'__raw__', args:[raw] };
    }
    default: return { behavior:'__raw__', args:[g('le-custom-val') || ''] };
  }
}

function leUpdatePreview() {
  const b = leCollectBinding();
  const el = document.getElementById('le-preview');
  if (el) el.textContent = leBindingToString(b) || '—';
}
...
function leApply() {
  ...
  const newContent = serializeDtsiLayerBindings(leState.bindings, file.content);
  file.content = newContent;
  file.modified = file.content !== file.original;
