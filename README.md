...
| BT_PERIPHERAL_PREF_MAX_INT | **R側=6 (7.5ms 固定)** / L側=12 (15ms) | R・L | **〈Cursor Cadence Experiment〉(2026-06-13)**: R側(ホスト向け central)のみ 6/6 固定で 7.5ms (≈133Hz) を狙いカーソル平滑化を試行。Apple 機で ≈30ms 転落の既知リスクあり → 悪化時は R側 MAX_INT を 12 へ戻す。L側(スプリット内リンク)は 6/12 据え置き |
...
| DATE | ENTRY |
|---|---|
| 2026-06-13 | 〈Cursor Cadence Experiment · Apple Compat Wager〉— 素体 Cygnus `f076677` の BLE カーソル平滑化（`PREF_MAX_INT 12→6` で 7.5ms 固定 ≈133Hz）を**実験的に試行**（`feature/cursor-cadence-experiment` ブランチ限定。main は 6/12 を堅持）。右手〈Night_Sky_Sword〉(ホスト向け central) のみ `CONFIG_BT_PERIPHERAL_PREF_MAX_INT 12 → 6` とし、`MIN=MAX=6` 固定でホストへの報告周期を 7.5ms に張り付け、高速移動時のカーソルのカクつき低減を狙う。**これは 2026-05-27〈Bilateral BLE Trinity · Apple HID Interval Compat〉で `6→12` に緩めた判断を意図的に巻き戻す賭け**であり、当時の記録通り Apple BLE (mac/iPad/iOS) が範囲狭すぎとして 6/6 を拒否し ≈30ms (≈33Hz) へ転落する既知リスクを伴う（実機が iPad 主体のため転落公算は高め）。そのため **(1)** 左手〈Blue_Rose_Sword〉のスプリット内リンクは `6/12` 据え置き（カーソル平滑化に無関係なため変数を増やさない）、**(2)** conf コメントに「Apple 機で悪化を感じたら MAX_INT を 12 へ戻す」退路を明記、**(3)** リポルールに従い実験的変更として `feature/` ブランチ → 隔離 CI ビルドで .uf2 を試走、の三点で安全に検証する方針。なお本ブランチは先行の〈EXPERIMENTAL_CONN Dissolution〉(`5a94931`) を内包し、iPad ペアリング修正を保持したまま 6/6 のみを上乗せする。良好なら main へ昇華、悪化なら PR を閉じて 6/12 を堅持する。 |
