...
| DATE | ENTRY |
|---|---|
| 2026-05-28 | 〈Bond Overwrite Sanction · New Pairing Restoration〉— 「無線が新規追加ペアリングできない（1台目は pair できるが2台目以降/再ペア不可）」症状を浄化。 〈Pure Pairing Sigils〉(`d0c5ec6`) で `bt_pair_N`（純 `&bt BT_SEL N`）を投入しても解決しなかったのは、真因が keymap/マクロ側ではなく **central 側 conf のセキュリティフラグ欠落**だったため。 ZMK は 3 つの実験的 BLE フラグを持ち（公式 `docs/config/bluetooth.md` で確認）、**`CONFIG_ZMK_BLE_EXPERIMENTAL_SEC`** が "allows overwrite of keys from previously paired hosts"（既存ペア済みホストの鍵上書き許可 = `BT_SMP_ALLOW_UNAUTH_OVERWRITE` を imply）でペアリング問題に最も関連する。 従来 conf は `CONFIG_ZMK_BLE_EXPERIMENTAL_CONN=y`（接続安定性のみ）だけで `SEC` が無く、host 識別子に既存 bond が残るとき新規ペアリングが Zephyr SMP 層で拒否されていた。 修正: host bond を管理する central（`Night_Sky_Sword.conf`）に `CONFIG_ZMK_BLE_EXPERIMENTAL_SEC=y` を追加（`CONN`+`SEC` で `EXPERIMENTAL_FEATURES` 相当）。 キーマップ・`bt_pair`/`bt_solo` マクロ・プロファイル数（5）・bt_pair 配置は全て正常で、〈Memory Inscription Rollback〉も無関係（純粋に conf 1 フラグの欠落）。 双子リポ（42キー Cardinal）にも同欠落があったため `Elucidator.conf` へ予防同期。 ⚠ 実機 BLE 動作は CI で検証不可（ZMK 公式ドキュメント裏付けの config 修正）。 フラッシュ後、既存 bond が不調なら `&bt BT_CLR_ALL` / settings_reset で全消去 → 再ペアが確実。 これにより〈Administrator〉が複数ホストへの新規 bond 確立権能を取り戻す。 |
