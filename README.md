...
| BT_PERIPHERAL_PREF_LATENCY（ホスト接続） | 未設定（Zephyr既定30） | R側（Central、ホスト接続） | ホストとの接続でデータ無しの接続イベントを何回スキップしてよいか。従来 0（毎接続イベント必須応答）だったが、〈Idle Tax Removal · Administrator Sync〉（2026-09-12）で設定ごと撤去し既定30へ委ねた。0だとホストのBLEスタックがアイドル時も休めず常時稼働し、PC側が他処理で混み合うとキー入力・トレーラボール移動が両方遅延する症状の主要容疑。42キー版〈Cardinal〉の同名修正への双子同期（42キー版では実機検証済み）。左右分割リンク側の `ZMK_SPLIT_BLE_PREF_LATENCY`（Split BLE Latency 行、下記）とは別物でホストとは無関係のため変更していない |
