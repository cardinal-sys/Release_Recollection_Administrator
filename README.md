...
| BT_GAP_AUTO_UPDATE_CONN_PARAMS | 未設定（Zephyr既定の有効のまま） | R・L両側 | 接続後に自動パラメータ再交渉。既定が既に有効のため明示指定を撤去（無変化）。42キー版〈Cardinal〉に双子同期（2026-09-14） |
| BT_CONN_PARAM_UPDATE_TIMEOUT | 未設定（Zephyr既定5000ms） | R・L両側 | 〈Conn Param Timeout Sigil Removal · Administrator Sync〉（2026-09-14）。従来1000ms（接続から1秒後にパラメータ更新要求）はBluetooth Core仕様「Peripheralは接続確立後5秒間は接続パラメータ更新手続きを行うべきではない」に違反。出典元のZMK本家Issue #3381（sub-7.5ms低遅延スプリットリンク向けレシピ）は投稿者自身が「Bluetoothスペック非準拠、Zephyrデバイス間のみで機能する」と明記しており、ホスト向け接続への転用は不適切だった疑い。42キー版〈Cardinal〉で実機検証済み（Mac/iPad1・2台目のペア成功）の修正を双子同期 |
| BT_CTLR_DATA_LENGTH_MAX / BT_BUF_ACL_RX・TX_SIZE | 未設定（Zephyr既定27） | R・L両側 | 〈Data Length Sigil Removal · Administrator Sync〉（2026-09-14）。従来251はZephyr本家Issue #38782で「Windows/タブレット/非SnapdragonスマホでBT_HCI_ERR_CONN_TIMEOUTを伴うペアリング失敗を招く既知の互換性問題」と判明した値とほぼ同一。原型Cygnus-S-Lkeymouseにも存在せず既定へ委ねる。42キー版〈Cardinal〉に双子同期 |
| BT_CTLR_PHY_2M / BT_AUTO_PHY_UPDATE | 未設定（Zephyr既定のyのまま） | R・L両側 | 〈Redundant Sigil Cleanup · Administrator Sync〉（2026-09-14）。原型が使う古いZephyr `v3.5.0+zmk-fixes` でも既定はyと確認済みで、明示指定は実効果ゼロと判明したため可読性目的でコメントアウトのみ実施。42キー版〈Cardinal〉に双子同期 |
| BT_PERIPHERAL_PREF_TIMEOUT | 未設定（Zephyr既定） | R・L両側 | ホスト向け supervision timeout。従来 1000（10秒、Apple上限6秒を超過）だったが、〈Timeout Sigil Removal · Administrator Sync〉（2026-09-11）で設定ごと撤去し既定値へ委ねた。42キー版〈Cardinal〉の同名修正への双子同期 |
| BT_PERIPHERAL_PREF_LATENCY（ホスト接続） | 未設定（Zephyr既定30、R・L両側） | R側（Central、ホスト接続）／L側（split link） | ホストとの接続でデータ無しの接続イベントを何回スキップしてよいか。従来 0（毎接続イベント必須応答）だったが、〈Idle Tax Removal · Administrator Sync〉（2026-09-12）でR側のみ設定ごと撤去し既定30へ委ねた。0だとホストのBLEスタックがアイドル時も休めず常時稼働し、PC側が他処理で混み合うとキー入力・トレーラボール移動が両方遅延する症状の主要容疑。42キー版〈Cardinal〉の同名修正への双子同期（42キー版では実機検証済み）。**L側（Blue_Rose_Sword）は当時撤去対象から漏れており2026-09-16〈Idle Tax Removal · Bilateral Sync〉で是正**（L側はsplit link向けの設定でホスト接続とは無関係だが、「左右完全一致」の方針に合わせ撤去）。左右分割リンク側の `ZMK_SPLIT_BLE_PREF_LATENCY`（Split BLE Latency 行、下記）とは別物でホストとは無関係のため変更していない |
...
| ZMK_SLEEP（Deep Sleep） | **無効化**（従来有効、R・L両側） | R・L両側 | 〈Deep Sleep Sigil Removal · Administrator Sync〉（2026-09-14）。ZMK本家Issue #3195「Central side does not wake from sleep when ZMK Studio is enabled (Zephyr 4.1)」の既知バグ条件（`CONFIG_ZMK_STUDIO=y`+`CONFIG_ZMK_SLEEP=y`、本構成が固定するZephyrはちょうど4.1.0）に一致していたため撤去。原型Cygnus-S-Lkeymouseも`CONFIG_ZMK_SLEEP`自体を設定せず既定（Deep Sleep無効）のまま。42キー版〈Cardinal〉に双子同期。長時間放置時の追加省電力は失うがIDLE（5分）は維持 |
...
| PMW3610 force-awake | **再有効化**（2026-09-13〜09-17は無効化） | 〈Wakeful Vigil Suspension · Administrator Sync〉（2026-09-13）でバッテリー%変動切り分けのため無効化していたが、〈Silent Vigil Recall · Administrator Sync〉（2026-09-17）でBLE再接続直後にトラックボールのみ無反応になる症状（42キー版〈Cardinal〉と共通で発生確認済み）の切り分け実験として双子同期で再有効化。原因は未確定（実機シリアルログ未採取）で一変数の仮説検証に留まる。効果がなければ再度無効化に戻す想定 |
...
| 2026-08-03 | 〈Known-Good Anchor · Administrator Sync〉— 42キー〈Cardinal〉で Mac との BLE 不通（ペアリング後に切断 → やがて接続不能）の真因と判明した **`config/west.yml` の浮動参照**を、50キー〈Administrator〉側でも先回りで封印する双子同期（Cardinal 側 `423a992` +`3e3fd32`）。**構造的欠陥**: `zmk` が `revision: main`、`zmk-listeners` と `zmk-scroll-snap` が `revision: v1`（**タグではなくブランチ**）で、さらに zmk の `app/west.yml` が zephyr を `v4.1.0+zmk-fixes`（これもブランチ）で指すため、**リポジトリが同一コミットでもビルドの度に上流が入れ替わる**状態だった。**実測で漂流を確認済み**: 2026-06-13 (run `27458367283`) は zmk=`ff09f2d` / zephyr=`58a5874a` / scroll-snap=`1f8fc86` だったが、2026-07-06 (run `28760241281`, "Adjust encoder steps and triggers per rotation") では zmk=`484a054` / zephyr=`9df4b12b` / scroll-snap=`dd21a0f` へ移動（ビルド時間も 2m34s → 5m37s とキャッシュミス）。**前者は Cardinal #227（実機で無線動作を確認できた唯一の基準点）と完全に同一の上流であり、後者は Cardinal #228（不通側）と同じ側**。**対処**: 4 件を 2026-06-13 時点の実コミットへ固定 ―― zmk `main` → `ff09f2d0…`、zmk-listeners `v1` → `c240cea`、zmk-scroll-snap `v1` → `1f8fc86`、および `zephyr` を同名 project 再定義で上書きして `58a5874a…` へ（west の「取り込む側の manifest が優先」する解決順を利用。`import.name-blocklist` 17 項目は zmk 側 `ff09f2d` と機械照合で完全一致を確認、`clone-depth: 1` は古い SHA の浅い取得を避けるため意図的に非複製）。適用後、**双剣の west.yml revision は全 12 project で完全一致**。キーマップ・conf・エンコーダ設定には一切触れていない。なお 2026-07-06 のエンコーダ調整コミットは SYSTEM LOG へ未記載のまま残っている（別途整合が必要）。 |
